...
for i in range(0,n_splits*(train_len+1000),train_len+1000):
    ...
    model = nlfea(test_len=test_len,n=8,degree=2,k=k,reg=1e-3)
    model.fit(X_train,y_train,X_train_delay)
    y_pred = model.predict(X_test[0],X_test_delay).T
    y_pred = scalar.inverse_transform(y_pred)
    mse = mean_squared_error(y_test,y_pred)
    print(f'mse of {i/4000+1} fold, from traindata({i}-{i+train_len}) {mse}')
    mse_lst.append(mse)
