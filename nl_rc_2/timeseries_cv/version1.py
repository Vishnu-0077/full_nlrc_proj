...
n_fold = 20
mse_lst = []
for i in range(base_train_len,base_train_len+n_fold*test_len,test_len):
    X_train = data[k:i+k]
    X_delay_train = data[:k]
    X_test = data[i+k:i+k+test_len]
    X_delay_test = data[i:i+k]

    y_train = data[k+1:i+k+1]
    y_test = data[i+1+k:i+1+k+test_len]
    ...
    model = nlfea(test_len=test_len,degree=2,k=k,n=3,reg=1e-4)
    model.fit(X_train,y_train,X_delay_train)
    y_pred = model.predict(X_test[0],X_delay_test).T
    y_pred = scalar.inverse_transform(y_pred)
    mse = mean_squared_error(y_test,y_pred)
    mse_lst.append(mse)
    print(f'mse of fold, from traindata({0}-{i}) {mse}')

final = sorted(mse_lst)[:n_fold-5]
print(np.mean(final))
