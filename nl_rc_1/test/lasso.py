...
class nlfea:
    ...
    def fit(self,data,y_data,delay_X_train):
        ...
        self.lasso = Lasso(max_iter=10000,alpha = reg,fit_intercept=False)
        self.lasso.fit(history_x,yt)
        self.w_out = self.lasso.coef_
        return self
    ...
    def predict(self,u,delay_X_test):
        ...
        for i in range(test_len):
            ...
            poly_feat = self.poly.transform(lin.reshape(1,-1))
            poly_feat = self.feature_scalar.transform(poly_feat.reshape(1,-1))[0]
...
def tent_map(mu=2.0, x0=0.5422, length=2000):
    x = np.zeros(length)
    x[0] = x0
