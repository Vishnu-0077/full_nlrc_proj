...
class rc_nl:
    ...
    def __init__(self,n=5,ressize=100,spectral_radius=0.95,leak_rate=0.3,reg=1e-8,test_len=10,init_len=100,seed=42):
        ...
        self.W_in = np.random.rand(ressize,4) - 0.5
    ...
    def build_features(self,data):
        ...
        for i in range(train_length):
            u = data[i].item()
            ss = self.neuron_iterator(u)
            ene = self.energy(ss)
            var = self.variance(ss)
            self.x = self.x*(1-a)+a*np.tanh(np.dot(W,self.x)+np.dot(W_in,np.vstack((1,u,ene,var))))
            if i>=init_len:
    ...
    def predict(self,u):
        ...
        for i in range(self.test_len):
            ...
            ene = self.energy(ss)
            var = self.variance(ss)
            x = x*(1-a)+a*np.tanh(np.dot(W,x)+np.dot(W_in,np.vstack((1,u,ene,var))))
...
res_lst = [100,150,200,300,400,500]
mse_list = {}
for ressize in res_lst:
    ...
    for n in n_lst:
        for reg in reg_lst:
            ...
            reswise_mse[(reg,n)] = mse
    print('--for ressize',ressize,'--')
    print(sorted(reswise_mse.items(),key=lambda x:x[1])[:5])
    print()
